# 校园 BBS Wiki · 长文与切片 2.0

把校园 BBS 中的故事、经验、人物、事件与社区词语整理为可检索、可追溯的 Wiki。正式应用已经接入自己的后端和 SQLite 数据库；读者可以在 **页外 MARGIN** 与 **游离 DRIFT** 两套阅读主题间切换。长文与短内容共用编辑、导入、版本记录与 MCP，但可以投放到不同信息流。

## 本次适配

| 信息流 | 路径 | 默认内容 |
| --- | --- | --- |
| 发现 `main` | `/f/main` | 长文与切片混排 |
| 长文 `wiki` | `/f/wiki` | 只接收长文 |
| 切片 `slices` | `/f/slices` | 只接收短内容 |

后台「站点与账号」设置每个流允许的内容类型、启用状态和默认首页；条目编辑器设置具体投放位置。已发布、已投放且类型匹配的条目才进入该流。`feedIds: []` 表示不投放，不代表私密：已发布内容仍可从全站索引和永久链接访问。草稿或归档才会阻止公开访问。

切片支持原话、原图、前后对照、行为概述。原话、背景、来源和更正分别存储，首屏不再重复展示标题与摘要。长文保持 Markdown 阅读。

**已有站点先看 [升级说明](UPGRADE_V2.md)**；MCP 接入见 [MCP 文档](docs/MCP.md)，实际验证范围见 [测试报告](QA_REPORT.md)。这份交付是源码，不含现网数据库、上传文件、凭据或生产构建产物。
...
`setup` 在 `.env` 不存在时创建配置，生成互不相同的随机管理员密码与 MCP_TOKEN；已有配置使用 `npm.cmd run setup -- --upgrade` 补齐新字段，不覆盖已有值。服务启动时自动创建数据库、迁移表结构，并在空库中创建管理员。**初始内容库为空**；请到后台新增或导入资料。
...
`margin` 是暖白纸页主题，`drift` 是深色阅读主题；访客选择优先于站点默认主题。`BBS_THREAD_URL_TEMPLATE` 可留空；只有批量导入的记录缺少 `sourceUrl` 时，才会由帖子 ID 生成原帖地址。

配置字段、后端地址、跨域、管理员密码及生产环境的完整说明见 [配置文档](docs/CONFIGURATION.md)。不要把 `.env` 提交到版本库。

## 已提供的功能

- 两套主题、长短内容分别排版、响应式布局、主题记忆、原生滑动和键盘浏览；切换主题保留当前条目。
- 独立 Bearer 凭据的 MCP：15 个工具，支持只读管理、局部更新、信息流配置、批量预览提交和媒体上传。
...
这里的 **artifact 就是一条长文或切片**，可以是校园故事、指南、人物、事件或社区词语。后台只有管理员账号，访客无需注册。访客收藏保存在当前浏览器中，不跨设备同步。

## 录入内容

进入后台 →「条目管理」→「新建条目」，先选择长文或切片，填写相应正文、原话或材料，勾选投放信息流；按需补充分类、标签、别名、原帖出处、图片与附件，再保存草稿或发布。
