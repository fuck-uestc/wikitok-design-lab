...
import { api, isAbort, messageOf, RequestError } from '../api';
...
export function useFeed(category: string, feed: string, format: string) {
  ...
  useEffect(() => {
    ...
    setState(initial);
    const params = new URLSearchParams({ feed, ...(format ? { format } : {}), limit: '12', ...(category ? { category } : {}) });
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      const params = new URLSearchParams({ feed, ...(format ? { format } : {}), limit: '12', cursor: state.cursor, ...(category ? { category } : {}) });
      ...
      if (!isAbort(error) && generation.current === requestGeneration) {
        // A configuration edit invalidates cursor scope. Retrying the same cursor cannot recover.
        if (error instanceof RequestError && error.code === 'INVALID_CURSOR') setReload(n => n + 1);
        else setState(s => ({ ...s, loadingMore: false, moreError: messageOf(error) }));
      }
    }
    finally { if (moreRequest.current === controller) moreRequest.current = null; }
  }, [category, feed, format, state.cursor, state.loading]);
