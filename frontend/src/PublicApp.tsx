...
import type { ArtifactSummary, ContentFormat, ImagePresentation, Taxonomy, Theme } from '../../shared/types';
...
const IMAGE_PRESENTATION_KEY = 'campus-wiki:image-presentation:v1';
export default function PublicApp() {
  ...
  const [imagePresentation, setImagePresentation] = useState<ImagePresentation>(() => { try { const value = localStorage.getItem(IMAGE_PRESENTATION_KEY); if (value === 'immersive' || value === 'standalone') return value; } catch { /* Optional storage. */ } return 'immersive'; });
  ...
  function chooseImagePresentation(value: ImagePresentation) {setImagePresentation(value);try{localStorage.setItem(IMAGE_PRESENTATION_KEY,value);}catch{toast('图片样式仅在本次浏览中保留。');}}
  ...
  return <><a className="skip-link" href="#feed">跳到内容</a>{theme==='margin'?<MarginLayout {...layout}/>:<DriftLayout {...layout}/>}
    ...
    {showTheme&&<ThemeDialog theme={theme} imagePresentation={imagePresentation} onTheme={chooseTheme} onImagePresentation={chooseImagePresentation} onClose={()=>setShowTheme(false)}/>}
