...
const blank: ArtifactInput = { title: '', slug: '', summary: '', content: '', kind: 'article', category: '', tags: [], aliases: [], author: '', sourceUrl: '', sourceTitle: '', sourceThreadId: '', externalId: '', coverUrl: '', coverMediaId: '', coverAlt: '', coverCredit: '', eventDate: '', status: 'draft', attachmentIds: [] };
...
export function ArtifactEditor() {
  ...
  function restoreCache(cached: CachedDraft) { setForm(cached.form); setTagText(cached.tagText); setAliasText(cached.aliasText); setDirty(true); }
  useEffect(() => {
  ...
  return <form className="artifact-editor" onSubmit={submit} noValidate>
    ...
    <fieldset disabled={saving} className="editor-fields"><div className="editor-columns"><div className="editor-main-column">
      <section className="admin-panel editor-section"><label className="title-field">条目标题 <span>*</span><input name="title" placeholder="给这段记忆一个准确的名字" maxLength={200} value={form.title} onChange={event => update('title', event.target.value)} /></label><label>卡片摘要<textarea name="summary" rows={3} maxLength={600} value={form.summary} onChange={event => update('summary', event.target.value)} placeholder="用两三句话介绍这条内容，留空则从正文生成。" /></label><div className="editor-content-label"><label htmlFor="artifact-content">完整正文 <span>*</span></label><div className="editor-mode" role="group" aria-label="正文编辑模式"><button type="button" aria-pressed={!preview} onClick={() => setPreview(false)}><Pencil className="ico" />编辑</button><button type="button" aria-pressed={preview} onClick={() => setPreview(true)}><Eye className="ico" />预览</button></div></div>{preview ? <div className="editor-markdown-preview">{form.content ? <Markdown content={form.content} /> : <p>写下正文后，在这里预览排版。</p>}</div> : <textarea id="artifact-content" className="markdown-editor" name="content" spellCheck={false} maxLength={200000} value={form.content} onChange={event => update('content', event.target.value)} placeholder={'用 Markdown 整理故事、资料与线索。\n\n## 这是什么\n\n## 来龙去脉\n\n> 保留作者原意，为关键事实附上来源。'} />}<p className="field-hint">支持标题、列表、引用、表格和链接。HTML 与脚本不会作为页面执行。</p></section>
