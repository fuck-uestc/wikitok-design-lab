...
export const FeedViewport = forwardRef<FeedHandle, Props>(function FeedViewport({ theme, items, active, onActive, renderCard, empty }, handle) {
  ...
  return <main id="feed" ref={feed} className={`feed ${theme === 'margin' ? 'editorial' : 'immersive'}-feed`} tabIndex={-1} aria-label="校园知识流，向下滑动浏览" onScroll={() => {
    ...
    {!items.length ? empty : <>
      ...
      {items.slice(start, end).map((item, offset) => <article key={item.id} data-index={start + offset} data-artifact-id={item.id} className={`slide ${theme === 'margin' ? 'editorial' : 'immersive'}-slide`} style={{ height: height || '100%' }} inert={start + offset !== active} aria-hidden={start + offset !== active} aria-label={item.title}>{renderCard(item, start + offset)}</article>)}
