import { ShortReader } from './ShortReader';
...
export function LibraryDialog({ mode: initialMode, scope, savedIds, onClose, onRead, onSave }: { mode: 'all' | 'saved' | 'search'; scope: {feed:string;format:string}; savedIds: string[]; onClose: () => void; onRead: (artifact: ArtifactSummary) => void; onSave: (id: string) => void }) {
  ...
  const scoped = !everywhere ? {feed:scope.feed,...(scope.format?{format:scope.format}:{})} : {};
    ...
    const timer = setTimeout(async () => {
      try {
        if (mode === 'saved') {
          ...
          const result = await api<Page<ArtifactSummary>>(`/artifacts?${new URLSearchParams({ ...scoped, q: query, limit: '20' })}`, { signal: controller.signal });
          if (controller.signal.aborted) return;
  ...
  }, [mode, query, savedIds, reload, everywhere, scope.feed, scope.format]);
  async function more() {
    ...
    try {
      const result = await api<Page<ArtifactSummary>>(`/artifacts?${new URLSearchParams({ ...scoped, q: query, limit: '20', cursor })}`, { signal: controller.signal });
  ...
  return <Dialog title={initialMode === 'search' ? '搜索条目' : mode === 'saved' ? '我的收藏' : '条目索引'} onClose={onClose}>
    <div className="library-intro"><div className="library-tabs"><button aria-pressed={mode === 'all'} onClick={() => setMode('all')}>全部条目</button><button aria-pressed={mode === 'saved'} onClick={() => setMode('saved')}>我的收藏</button></div><p>{mode === 'saved' ? '收藏保存在当前浏览器，已下架的条目暂不显示。' : '搜索标题、正文、作者、人物别名、标签或帖子 ID。'}</p>{mode !== 'saved' && <div className="search-scope"><label><input type="checkbox" checked={everywhere} onChange={e=>setEverywhere(e.target.checked)} />搜索全部公开条目（包含未投放到当前流的内容）</label></div>}<label className="search-box"><Search className="ico" /><input ref={search} type="search" value={query} onChange={event => setQuery(event.target.value)} maxLength={100} placeholder="搜索词条、人物或记忆片段" aria-label="搜索条目" /></label></div>
...
export function ReaderDialog({ slug, scope, savedIds, onSave, onClose, onRead }: { slug: string; scope: {feed:string;format:string}; savedIds: string[]; onSave: (id: string) => void; onClose: () => void; onRead: (artifact: ArtifactSummary) => void }) {
  ...
  useEffect(() => {
    ...
    api<Artifact>(`/artifacts/${encodeURIComponent(slug)}`, { signal: controller.signal }).then(async a => {
      setArticle(a);
      if (a.category) {
        try { const result = await api<Page<ArtifactSummary>>(`/artifacts?${new URLSearchParams({ feed: scope.feed, ...(scope.format?{format:scope.format}:{}), category: a.category, limit: '4' })}`, { signal: controller.signal }); setRelated(result.items.filter(item => item.id !== a.id).slice(0, 3)); } catch { /* The reader remains available when related entries fail. */ }
      }
    }).catch(failure => { if (!isAbort(failure)) setError(messageOf(failure)); });
    return () => controller.abort();
  }, [slug, reload, scope.feed, scope.format]);
  return <Dialog title="内容与材料" onClose={onClose} className="read-dialog">
    {!article ? error ? <div className="read-body"><ErrorNotice message={error} retry={() => setReload(n => n + 1)} /></div> : <Loading /> : <div className="read-body">
      {article.format === 'short' && article.short ? <ShortReader article={article}/> : <>
      ...
      {article.format !== 'short' && (article.sourceUrl || article.coverCredit) && <div className="source-block">{article.sourceUrl && <ExternalLink href={article.sourceUrl}>{article.sourceTitle || '查看 BBS 原帖'}{article.sourceThreadId && ` · #${article.sourceThreadId}`}</ExternalLink>}{article.coverCredit && <div className="credit">图像署名：{article.coverCredit}</div>}</div>}
      {article.attachments.some(file => !article.short?.sources.some(source => source.mediaId === file.id)) && <section className="reader-attachments"><h2>附件与资料</h2>{article.attachments.filter(file => !article.short?.sources.some(source => source.mediaId === file.id)).map(file => <a key={file.id} href={mediaUrl(file.url)} target="_blank" rel="noopener noreferrer"><FileText className="ico" /><span>{file.originalName}<small>{Math.max(1, Math.round(file.size / 1024))} KB</small></span><ArrowUpRight className="ico" /></a>)}</section>}
      <div className="read-actions"><button className="pill-button" onClick={() => onSave(article.id)}><Bookmark className="ico" />{savedIds.includes(article.id) ? '移出我的收藏' : '收藏条目'}</button><span className="reader-version">第 {article.version} 版</span></div>
...
export function ShareDialog({ url, onClose }: { url: string; onClose: () => void }) {
  const { toast } = useApp();
  return <Dialog title="分享条目" onClose={onClose}><div className="share-body"><p>复制链接，邀请朋友一起读。</p><input className="copy-input" value={url} readOnly aria-label="条目分享链接" onFocus={event => event.currentTarget.select()} /><button className="pill-button" onClick={async () => {
