...
function LongCard({ artifact: a, index, saved, onRead, onSave, onShare, onImage }: CardProps) {
  const hasImage = !!coverUrl(a);
  return <div className={`entry-spread ${hasImage ? 'with-media' : 'text-only'}`}>
    ...
    {hasImage && <figure className="entry-media"><button onClick={onImage} aria-label="查看完整图片"><ArtifactImage artifact={a} eager={index < 2} /></button>{a.coverCredit && <figcaption>{a.coverCredit}</figcaption>}</figure>}
  </div>;
}
